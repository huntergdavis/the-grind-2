...
const runtimeArtifacts = Object.freeze(
  factualStoryBeatBrowserEvaluationRuntimeFiles.map((artifact) =>
    Object.freeze({
      ...artifact,
      file: resolve(
        repositoryRoot,
        `node_modules/onnxruntime-web/dist/${artifact.path}`,
      ),
    })),
);
...
function usage(message) {
  ...
  process.stderr.write(
    "Usage: node tools/narrator-story-beat-v2-browser-evaluation/run.mjs evaluate "
      + "--model-dir <exact-pinned-six-file-dir> "
      + "--holdout <sealed-v2-holdout.json> --run-id <id> "
      + "--out <fresh-dir-under-.narrator-t5-rebuild> [--full]\n",
  );
  process.exitCode = 2;
}
...
async function filesUnder(root, prefix = "") {
  ...
  for (const entry of entries.sort((left, right) =>
    compareFactualStoryBeatClosurePathSegments(left.name, right.name))) {
...
async function snapshotModel(requested) {
  ...
  if (canonicalStringify(paths)
    !== canonicalStringify(factualStoryBeatBrowserEvaluationModelPaths)) {
    throw new Error("Model directory must contain the exact six-file q8 closure");
  }
  const snapshot = await snapshotFiles(
    lexical,
    factualStoryBeatBrowserEvaluationModelPaths,
  );
  if (canonicalStringify(snapshot.files)
      !== canonicalStringify(factualStoryBeatBrowserEvaluationModelFiles)
    || snapshot.aggregateSha256
      !== factualStoryBeatBrowserEvaluationExpectedModelAggregateSha256) {
    throw new Error("Model directory differs from the retained factual V2 q8 pair");
  }
...
async function snapshotSource() {
  const commit = await assertCommittedSourceSnapshot({
    repositoryRoot,
    sourcePaths: factualStoryBeatBrowserEvaluationSourcePaths,
  });
...
async function snapshotHoldout(requested, full) {
  const lexical = isAbsolute(requested)
    ? resolve(requested)
    : resolve(process.cwd(), requested);
  const file = await regularFile(lexical, "Sealed factual holdout");
  const bytes = await readFile(file);
  const observedSha256 = sha256(bytes);
  if (observedSha256
    !== factualStoryBeatBrowserEvaluationExpectedHoldoutSha256) {
    throw new Error(
      "Sealed factual holdout SHA-256 does not match committed export evidence",
    );
  }
  const text = new TextDecoder("utf-8", { fatal: true }).decode(bytes);
  const envelope = parseSealedFactualStoryBeatHoldout(text);
  const selected = selectFactualStoryBeatHoldoutCases(envelope.cases, full);
  const indexes = Object.freeze(
    selected.map((row) => envelope.cases.indexOf(row)),
  );
  return Object.freeze({
...
async function resolveOutputTarget(requested, modelDirectory) {
  ...
  if (realIgnored !== ignoredRoot
    ...
    throw new Error(
      "Evaluation output must be a fresh directory beneath .narrator-t5-rebuild",
    );
  }
...
async function buildBundle() {
  const directory = await mkdtemp(
    resolve(tmpdir(), "grind2-factual-story-beat-v2-browser-evaluation-"),
  );
  try {
    await executeFile(resolve(repositoryRoot, "node_modules/.bin/vite"), [
      "build",
      "--config",
      resolve(toolRoot, "vite.config.ts"),
      "--outDir",
      directory,
    ...
    if (paths.length !== 4
      ...
      || paths.filter(
        (path) => /^assets\/index-[A-Za-z0-9_-]+\.js$/u.test(path),
      ).length !== 1
      || paths.filter(
        (path) =>
          /^assets\/transformers\.worker-[A-Za-z0-9_-]+\.js$/u.test(path),
      ).length !== 1
      || paths.filter(
        (path) =>
      ...
      throw new Error(
        "Factual story-beat browser bundle does not have the exact four-file layout",
      );
    }
...
async function startServer(bundle, model, runtime, holdout) {
  ...
  model.assets.forEach((asset, index) => staged.set(
    `/__factual_story_beat_v2_evaluation_staging__/model/${index}`,
    asset,
  ));
  runtime.assets.forEach((asset, index) => staged.set(
    `/__factual_story_beat_v2_evaluation_staging__/runtime/${index}`,
    asset,
  ));
  staged.set("/__factual_story_beat_v2_evaluation_staging__/holdout/0", {
    path: "sealed-holdout.json",
    bytes: holdout.bytes,
  });
  const bundled = new Map(
    bundle.assets.map((asset) => [`/${asset.path}`, asset]),
  );
  ...
  if (typeof address !== "object" || address === null) {
    throw new Error("Evaluation server address is invalid");
  }
  return Object.freeze({
...
async function runBrowser({ options, bundle, model, runtime, holdout }) {
  ...
  try {
    ...
    await page.waitForFunction(() =>
      globalThis.__theGrindFactualStoryBeatV2BrowserEvaluation?.protocolVersion
        === 2);
    const stage = {
      protocolVersion: factualStoryBeatBrowserEvaluationProtocolVersion,
      runId: options["run-id"],
      modelAggregateSha256: model.aggregateSha256,
      holdout: stageArtifact(
        holdout,
        `${server.origin}/__factual_story_beat_v2_evaluation_staging__/holdout/0`,
      ),
      modelArtifacts: model.files.map((artifact, index) =>
        stageArtifact(
          artifact,
          `${server.origin}/__factual_story_beat_v2_evaluation_staging__/model/${index}`,
        )),
      runtimeArtifacts: runtime.files.map((artifact, index) =>
        stageArtifact(
          artifact,
          `${server.origin}/__factual_story_beat_v2_evaluation_staging__/runtime/${index}`,
        )),
      selectedIndexes: holdout.indexes,
    };
    await page.evaluate(
      (request) =>
        globalThis.__theGrindFactualStoryBeatV2BrowserEvaluation.stage(request),
      stage,
    );
    if (externalRequests.length !== 0) {
      throw new Error("Browser staging attempted an external request");
    }
    offline = true;
    ...
    const result = await page.evaluate(
      (milliseconds) =>
        globalThis.__theGrindFactualStoryBeatV2BrowserEvaluation.run(
          milliseconds,
        ),
      timeoutMs,
    );
    await page.evaluate(() =>
      globalThis.__theGrindFactualStoryBeatV2BrowserEvaluation.dispose());
...
function createReceipt({
  ...
  const metrics = summarizeFactualStoryBeatResults(
    holdout.selected,
    browserResult.results,
  );
  if (metrics.validCaseCount !== metrics.caseCount
    || metrics.fallbackRequiredCaseCount !== 0
    || metrics.exactPlaceCaseCount !== metrics.caseCount
    || metrics.requiredClausesCompleteCaseCount !== metrics.caseCount
    || metrics.unknownLexemeCaseCount !== 0
    || metrics.promptScaffoldEchoCaseCount !== 0
    || metrics.sourceFieldExactEchoCaseCount !== 0) {
    throw new Error("Grounded factual browser quality gate failed");
  }
  ...
  const payload = {
    ...
    timing: {
      ...timingPayload,
      totalCaseElapsedMs:
        timingPayload.caseElapsedMs.reduce((sum, value) => sum + value, 0),
...
async function main(options) {
  ...
  if (holdout.sha256 !== holdoutAfter.sha256
    || holdout.corpusHash !== holdoutAfter.corpusHash) {
    throw new Error("Sealed factual holdout changed during the evaluation");
  }
  ...
  if (!verifyFactualStoryBeatBrowserEvaluationReceipt(receipt)) {
    throw new Error("Factual story-beat evaluation receipt failed verification");
  }
...
const options = parseFactualStoryBeatBrowserEvaluationArguments(
  process.argv.slice(2),
);
if (options === null) {
  usage("Invalid factual story-beat browser evaluation arguments.");
} else {
