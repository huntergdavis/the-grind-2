...
import {
  narratorBrowserEvaluationRpcVersionV3,
  type NarratorBrowserEvaluationCommandV3,
  type NarratorBrowserEvaluationResponseV3,
} from "../../../src/narrator/evaluation-browser-worker-port-v3";
import {
...
import {
  ...
  type NarratorTransformersCaseAdapterV3,
  type NarratorTransformersInputsV3,
...
} from "../../../src/narrator/evaluation-transformers-adapter-v3";
import { isNarratorEvaluationWorkerCaseRequestV3 } from "../../../src/narrator/evaluation-worker-protocol-v3";
import {
...
import { createNarratorVerifiedModelFetchV2 } from "../../narrator-browser-evaluation/src/verified-model-fetch";
...
let adapter: NarratorTransformersCaseAdapterV3 | null = null;
...
function caseRequestIsValid(value: unknown): boolean {
  if (runSpec === null
    || candidate === null
    || !isNarratorRecord(value)
    || !isNarratorRecord(value.eligibility)) return false;
...
function commandIsValid(value: unknown): value is NarratorBrowserEvaluationCommandV3 {
  ...
  if (value.kind === "initialize") {
    return narratorHasExactKeys(value, [
      ...
      && isNarratorEvaluationRunSpecV3(value.runSpec, value.candidate)
  ...
  if (value.kind === "run-case") {
    return narratorHasExactKeys(value, [
      ...
      && caseRequestIsValid(value.request);
  }
...
async function disposeRuntime(): Promise<void> {
  ...
  env.fetch = async () => {
    throw new Error("Narrator V3 browser worker is disposed");
  };
...
function runtimeProcessorBridge(processor: NarratorTrieLogitsProcessorV3): LogitsProcessorList {
  class NarratorTrieRuntimeProcessor extends LogitsProcessor {
    _call(inputIds: bigint[][], logits: unknown) {
      return processor.process(
        inputIds,
        logits as NarratorTransformersLogitsTensorV3,
      );
    }
  }
  const processors = new LogitsProcessorList();
  processors.push(new NarratorTrieRuntimeProcessor());
  return processors;
}

async function execute(command: NarratorBrowserEvaluationCommandV3): Promise<unknown> {
  ...
  if (command.kind === "handshake") {
    ...
    return createNarratorEvaluationWorkerBindingV3(identity.runSpec, identity.candidate);
  }
  ...
  if (command.kind === "load") {
    ...
    try {
      ...
      adapter = createNarratorTransformersCaseAdapterV3(
        identity.candidate,
        identity.runSpec,
        workerEpoch!,
        tokenizerPort,
...
workerScope.onmessage = (event: MessageEvent<unknown>): void => {
  ...
  void (async () => {
    let response: NarratorBrowserEvaluationResponseV3;
    try {
