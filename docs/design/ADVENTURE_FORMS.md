...
the visual explanation; there is no extra floating panel or narration request.

The full source chain is a separate collapsed disclosure in the existing Status
row. It names the controlling hero, the weakened enemy, the returned attack and
actual HP before/after. The final Last exchange remains unchanged: a useful control
action is not proof that it decided the battle. In the known actual journey,
Turning Check precedes Lantern Wolf 2's weakened Moonhowl, but Lantern Wolf 1 lands
the finishing blow one turn later. The hero then recovers normally.

Only a fully retained source application, surviving target, unexpired weakening
and matching direct retaliation qualify. Missing, refreshed, stale or foreign
evidence gets no new caption. Current-scene identity and already timestamped
terminal archives govern joining; a reusable combat ID alone is insufficient.
